...
import { ERC721CCompatibleContractsMock, ERC1155CMock } from "../typechain";
import { BigNumber } from "ethers";

let erc721c: ERC721CCompatibleContractsMock;
...
export const testERC721CCompatibleContracts = () => {
    before(async function () {
        // init contracts
        const ERC721C = await ethers.getContractFactory(
            "ERC721CCompatibleContractsMock"
        );
    ...
    describe("ERC721CCompatibleContracts", function () {
        ...
        it("Should remove a compatible contract", async function () {
            await erc721c.removeCompatibleContracts([erc1155c.address]);
            expect(await erc721c.getCompatibleContracts()).deep.equal([]);
        });
        ...
        it("Should fail to attach the previously detached component to a composable because component contract is no more compatible", async function () {
            await expect(
                erc721c.connect(acc1).attachComponent(1, erc1155c.address, 1, 1)
            ).to.be.revertedWith(
                "ERC721CCompatibleContracts: contract not compatible"
            );
        });
